...
import static com.jamint.ricette.JamPointCode.TipiCodici.ANGOLO_ROT;
...
//no) per spostamento<passo elemento spostato non fare i raccordi
    ...
    public int UdfVelLavRPM = 2500;
    public double UdfPuntiVelIni = 3d;
    public int UdfVelIniRPM = 300;
    public double UdfPuntiVelRall = 3d;
    public int UdfVelRallRPM = 200;
    public int Udf_FeedG0 = 10000;
    ...
    private static final String VALORE_CODE_ATTRIBUTE2 = "code-value2";
...
//                }
    ...
    public List<JamPointCode> checkInvalidCodes(boolean tryAdjustInvalidCodes)
    {
        ...
        for (JamPointCode code: codes)
        {
            ...
            if (step == null) {
                if(!code.tipoCodice.equals(ANGOLO_ROT))
...
*/
    ...
    public boolean addActiveStepCodeRotazione(JamPointCode.TipiCodici codeType, String codeValue1,String codeValue2) {
        return addStepCodeRotazione(getStep(activeStepIndex), codeType, codeValue1,codeValue2);
    }
    //AGGIUNGE UN CODICE AD UNO STEP SPECIFICATO ELETTRONICA M8
    private boolean addStepCodeRotazione(JamPointStep step, JamPointCode.TipiCodici codeType, String codeValue1, String codeValue2) {
        if (step!=null || codeValue2.equals("iniziale"))
        {
            saveUndo();
            JamPointCode code = new JamPointCode();
            code.tipoCodice = codeType;
            code.Angolo_rotaz = codeValue1;
            code.Num_punti_rotaz = codeValue2;
            code.step = step;
            codes.add(code);
            return true;
        }
...
//    }
        ...
        if (step!=null)
        {
            ...
            if (element.steps.size()!=0)
            {
                ...
                if (!isNextElementFeed && (!(isLastElement && isLastStepOfElement)))
                {
                    ...
                    double distance = Tools.getDistance(newLastStep, step); //DOPO LA CHIAMATA A splitElementAtStep step CONTIENE LE COORDINATE DELLO STEP APPENA SEPARATO, POSSO QUINDI TROVARE LA DISTANZA!
        ...
        while (eventType != XmlPullParser.END_DOCUMENT)
        {
            if (eventType == XmlPullParser.START_TAG)
            {
                ...
                else if (tagName.equals(CODE_TAG))
                {
                    ...
                    if(code.tipoCodice == ANGOLO_ROT) {
                        code.Angolo_rotaz =(xpp.getAttributeValue(null, VALORE_CODE_ATTRIBUTE));
                        code.Num_punti_rotaz = (xpp.getAttributeValue(null, VALORE_CODE_ATTRIBUTE2));
                    }else {
    ...
    public void  save(File file) throws IOException
    {
        ...
        for (JamPointCode code : codes)
        {
            ...
            if(code.tipoCodice == ANGOLO_ROT){
                xmlSerializer.attribute(null, VALORE_CODE_ATTRIBUTE, String.valueOf(code.Angolo_rotaz));
                xmlSerializer.attribute(null, VALORE_CODE_ATTRIBUTE2, String.valueOf(code.Num_punti_rotaz));
            }else {
                if (code.tipoCodice == SPEED_M8 || code.tipoCodice == TENS_M8)
    ...
    public void exportToUdf(File file)throws IOException
    {
        ...
        sb.append(newLine);
        sb.append(String.format ("%,.2f", pcX).replace(',', '.'));
        sb.append(newLine);
        sb.append(String.format ("%,.2f", pcY).replace(',', '.'));
        sb.append(newLine);
        sb.append(String.format ("%d", UdfVelLavRPM));
        sb.append(newLine);
        sb.append(String.format ("%,.3f", UdfPuntiVelIni/1000).replace(',', '.'));
        sb.append(newLine);
        sb.append(String.format ("%d", UdfVelIniRPM));
        sb.append(newLine);
        sb.append(String.format ("%,.3f", UdfPuntiVelRall/1000).replace(',', '.'));
        sb.append(newLine);
        ...
        List<JamPoint> points = getPoints();
        //imposto angolo iniziale rotazione come prima riga del udf
        for (JamPointCode code : codes) {
            try {
                if (code.Num_punti_rotaz.equals("iniziale")) {
                    sb.append("1010 " + code.Angolo_rotaz + " 0.000");
                    sb.append(newLine);
                }
            }catch (Exception e){}
        }
        ...
        for(JamPoint point: points)
        {
            try {
                ...
                if (point.element instanceof ElementLine || point.element instanceof ElementArc || point.element instanceof ElementZigZag) {
                    if (ListCodici.size()>0){
                        for(JamPointCode code:ListCodici ){
                            switch (code.tipoCodice){
                                ...
                                case RASAFILO:
                                    if(code.valore == JamPointCode.TipiValori.VALUE0)
                                        sb.append("2763 14.00 0.00");   //VDC13 = 0
                                    else
                                        sb.append("2763 14.00 1.00");   //VDC13 = 1
                                    sb.append(newLine);
                                    break;
                                ...
                                case ANGOLO_ROT:
                                    // lo metto dopo la riga delle coordinate XY
                    ...
                    if(jp.isDuplicateStep)
                        continue;

                    sb.append("1001 " + X + " " + Y);       //riga con quote X e Y


                    Element element = point.element;
                    if(isLastStepOfElement((JamPointStep)point)){
                        Element element_next = getNextElement(element);
                        if(element_next instanceof  ElementFeed) {
                            sb.append(newLine);
                            sb.append("2830 0 0");     ///fermata temporanea
                            sb.append(newLine);
                            sb.append("2750 4035 0");     ///per scartafilo
                           // sb.append(newLine);
                        }
                    ...
                    /*
                    //start// scartafilo o pinzafilo
                    Element element = point.element;
                    if( (element.pEnd.x ==((JamPointStep) point).p.x) && element.pEnd.y == ((JamPointStep) point).p.y) {
                        boolean lastElement = isLastElement(element);
                        if(lastElement) {
                            sb.append(newLine);
                            sb.append("2830 0 0");     ///fermata temporanea
                            sb.append(newLine);
...
*/
                            ...
                            switch (code.tipoCodice) {
                                case ANGOLO_ROT:
                                      ...
                                      if(code.Num_punti_rotaz.equals("iniziale")){
                                          ...
                                          sb.append(newLine);
                                          sb.append("1011 " + code.Angolo_rotaz + " " + code.Num_punti_rotaz);
                                      }
                ...
                if (point.element instanceof ElementFeed) {
                    /*
                    Element element = point.element;
                    if( !(element.pStart.x == pcX && element.pStart.y == pcY)) {
                        sb.append("2830 0 0");     ///fermata temporanea
                        sb.append(newLine);
                        sb.append("2750 4035 0");     ///per scartafilo
                        sb.append(newLine);
